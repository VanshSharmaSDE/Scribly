...
class AIService {
  ...
  setApiKey(apiKey) {
    ...
    // Clear any cached data and reinitialize completely
    this.removeApiKey();
  ...
  async testApiKey(apiKey) {
    try {
      ...
      if (error.message?.includes('API_KEY_INVALID') || error.message?.includes('invalid_api_key')) {
        ...
        throw new Error('API quota exceeded. Please check your usage limits or try a different API key.');
  ...
  async generateNote(prompt, options = {}) {
    if (!this.isInitialized()) {
      throw new Error('AI service not initialized. Please set your Google Gemini API key.');
    }
...
USER REQUEST: ${prompt}`;
      ...
      if (error.message?.includes('API_KEY_INVALID')) {
        ...
        throw new Error('API quota exceeded. Please check your Google Gemini usage limits.');
  ...
  selectEmojiFromContent(text, prompt) {
    ...
    const lowerContent = (String(prompt || '') + ' ' + String(text || '')).toLowerCase();
  ...
  validateNoteData(noteData) {
    return {
      ...
      tags: Array.isArray(noteData.tags) ? 
        noteData.tags.slice(0, 5).map(tag => String(tag).toLowerCase().replace(/[^a-z0-9\s-]/g, '').trim()) :
        ['ai-generated'],
  ...
  async generateContentFromTitle(title, options = {}) {
    ...
    try {
      ...
      if (error.message?.includes('API_KEY_INVALID') || error.message?.includes('invalid_api_key')) {
        ...
        throw new Error('API quota exceeded. Please wait or try a different API key.');
      } else if (error.message?.includes('RATE_LIMIT_EXCEEDED') || error.message?.includes('rate')) {
        throw new Error('Rate limit exceeded. Please wait a moment and try again.');
      } else if (error.message?.includes('PERMISSION_DENIED')) {
  ...
  createContentPrompt(title, noteType, language, tone, length) {
    ...
    const typeGuidance = {
      'meeting': 'Create meeting notes with agenda, discussion points, and action items',
      'project': 'Write about project overview, objectives, timeline, and milestones',
      'research': 'Structure with background, methodology, findings, and conclusions',
...
## Overview
This note covers important information about ${keywords.slice(0, 2).join(' and ')}.
...
`;
  ...
  extractKeywords(title) {
    const commonWords = ['the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'is', 'are', 'was', 'were', 'be', 'been', 'have', 'has', 'had', 'will', 'would', 'could', 'should'];
...
Return only the tags as a JSON array: ["tag1", "tag2", "tag3"]`;
      ...
      if (jsonMatch) {
        ...
        if (tags.length === 0) {
          ...
          if (bulletMatch) {
            tags = bulletMatch.map(item => item.replace(/[-•]\s*/, '').trim());
          } else {
            const commaMatch = text.match(/[a-zA-Z\s-]+/g);
            if (commaMatch) {
              tags = commaMatch.filter(item => item.trim().length > 2 && item.trim().length < 20);
            }
          }
        }
  ...
  generateSmartTagsFallback(title, content) {
    ...
    const titleWords = String(title || '').toLowerCase().split(/\s+/);
