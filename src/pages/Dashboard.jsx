...
const NoteCard = ({ note, onDelete, onToggleStar, viewMode = "grid" }) => {
  ...
  const customStyle = note.customStyle
    ? typeof note.customStyle === "string"
      ? JSON.parse(note.customStyle)
      : note.customStyle
    : {
            ...
            <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-all duration-300">
              <div className="flex items-center bg-gray-800/80 backdrop-blur-sm rounded-lg border border-gray-600/50 shadow-lg">
                <Link to={`/notes/view/${note.$id}`} className="block">
                  <div
                    className="p-2 text-gray-400 hover:text-blue-400 hover:bg-blue-400/10 transition-all duration-200 rounded-l-lg border-r border-gray-600/30"
                    title="View Note"
                  >
                ...
                <Link to={`/notes/edit/${note.$id}`} className="block">
                  <div
                    className="p-2 text-gray-400 hover:text-green-400 hover:bg-green-400/10 transition-all duration-200 border-r border-gray-600/30"
                    title="Edit Note"
                  >
  ...
  return (
    ...
    >
      <div className="bg-gray-900/60 backdrop-blur-sm border border-gray-700/50 rounded-xl p-6 h-full flex flex-col transition-all duration-300 hover:bg-gray-800/70 hover:border-blue-600/30 hover:shadow-xl hover:shadow-blue-600/10">
        ...
        <h3 className="text-lg font-semibold text-gray-100 mb-3 line-clamp-2 group-hover:text-white transition-colors">
          {note.title}
        </h3>
...
const Dashboard = () => {
  ...
  const { localAIStatus, initializeLocalAI, checkStatus, setLocalAIStatus } = useLocalAI(); // Get all LocalAI functions
  ...
  // Pre-check AI configuration for faster modal opening (without auto-initialization)
  useEffect(() => {
    const checkAIConfiguration = async () => {
      try {
        ...
        const hasLocalModel =
          userSettings.aiProvider === "local" && userSettings.localModelPath;

        setIsAIConfigured(hasGeminiKey || hasLocalModel);
        setAIConfigurationChecked(true);

        // Note: Auto-initialization removed per user request
        // Users can manually initialize LocalAI through the UI if needed
      } catch (error) {
  ...
  const handleAIGenerate = async () => {
    // Use pre-checked configuration for instant modal opening
    if (!aiConfigurationChecked || !isAIConfigured) {
      setShowAIGuide(true);
      return;
    }
    ...
    return (
      <>
        <ProfessionalBackground>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-16 min-h-screen">
            {/* Header with Profile */}
            <div className="flex flex-col lg:flex-row items-start justify-between mb-8 lg:mb-12 gap-4 lg:gap-0">
              <div className="flex-1 w-full lg:w-auto">
                ...
                >
                  ...
                  <motion.div
                    ...
                    style={{
                      background:
                        "linear-gradient(135deg, rgba(147, 51, 234, 0.1) 0%, rgba(79, 112, 226, 0.1) 100%)",
              ...
              <motion.div
                ...
                className="flex-shrink-0 w-full lg:w-auto lg:ml-8 order-first lg:order-last"
              >
            ...
            >
              <div className="max-w-md mx-auto px-4">
                ...
                <h3 className="text-xl sm:text-2xl font-semibold text-gray-200 mb-2 sm:mb-3">
                  No notes yet
                </h3>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 mb-4 sm:mb-6">
                  <Button
                    ...
                    disabled={!aiConfigurationChecked}
                    className="w-full sm:w-auto bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white border-0 shadow-lg shadow-purple-500/20 hover:shadow-purple-500/30 transition-all duration-300 hover:scale-105 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed px-4 sm:px-6 py-2 sm:py-3 text-sm sm:text-base"
                  >
                    <Sparkles className="h-4 w-4 mr-2" />
                    {aiConfigurationChecked ? "Create with AI" : "Loading..."}
                  </Button>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3 mb-4 sm:mb-6">
                  <button
                    ...
                    className="text-blue-400 hover:text-blue-300 text-sm flex items-center space-x-2 transition-colors hover:underline px-2 py-1"
                  >
        ...
        {/* AI Features Guide */}
        {showAIGuide && (
          <AIFeaturesGuide onClose={() => setShowAIGuide(false)} />
        )}
      </>
    );
  }
    ...
    <>
      <ProfessionalBackground>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-16 min-h-screen">
          ...
          >
            <div className="bg-gradient-to-r from-purple-900/20 to-pink-900/20 border border-purple-500/30 rounded-xl sm:rounded-2xl p-4 sm:p-6 backdrop-blur-sm">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-start sm:items-center space-x-3 sm:space-x-4">
                  <div className="p-2 sm:p-3 bg-gradient-to-r from-purple-600/30 to-pink-600/30 rounded-lg sm:rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-gray-300">
                      Generate intelligent notes with just a prompt
                    </p>
                  </div>
                </div>
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-3 w-full sm:w-auto">
                  <Button
                    onClick={handleAIGenerate}
                    disabled={!aiConfigurationChecked}
                    className="w-full sm:w-auto bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white border-0 shadow-lg shadow-purple-500/20 hover:shadow-purple-500/30 transition-all duration-300 hover:scale-105 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed px-3 sm:px-4 py-2 text-sm sm:text-base"
                  >
            ...
            <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 flex-1 w-full lg:w-auto">
                <div className="relative flex-1 max-w-full lg:max-w-md group">
                  <Search className="absolute left-3 sm:left-4 top-1/2 transform -translate-y-1/2 h-4 w-4 sm:h-5 sm:w-5 text-gray-400 group-focus-within:text-blue-400 transition-colors duration-300 z-10" />
                  <input
                    type="text"
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-3 sm:space-y-0 sm:space-x-4 w-full lg:w-auto">
                ...
                <div className="hidden sm:flex bg-gray-800/60 backdrop-blur-sm border border-gray-600/50 rounded-xl p-1 shadow-lg">
                  <button
              ...
              <div
                className={
                  viewMode === "grid"
                    ...
                    : "grid grid-cols-1 gap-3 sm:space-y-4 sm:block"
                }
              >
                <AnimatePresence>
      ...
      {/* AI Features Guide */}
      {showAIGuide && <AIFeaturesGuide onClose={() => setShowAIGuide(false)} />}
